...
int lis2dh_init(const struct device *dev)
{
	...
	/* Set up power pin for sensor */
	#define PWR_NODE DT_ALIAS(pwr)
	/* Define delay time for power on/off */
	#define POWER_DELAY_MS 100

    static const struct gpio_dt_spec pwr = GPIO_DT_SPEC_GET(PWR_NODE, gpios);
	...
	ret = gpio_pin_configure_dt(&pwr, GPIO_OUTPUT);
	if (ret < 0) {
		return 0;
	}

    ret = gpio_pin_toggle_dt(&pwr);
    if (ret < 0) {
        return 0;
    }
    k_msleep(POWER_DELAY_MS);
